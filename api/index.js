...
function getSupabase() {
    if (!_supabase) {
        _supabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
    }
    return _supabase;
}
// ============ HELPERS ============
...
function err(res, status, message) { return json(res, status, { success: false, message }); }
...
function getGeminiKeys() {
    ...
    if (process.env.GEMINI_API_KEY_FALLBACK)
        keys.push(process.env.GEMINI_API_KEY_FALLBACK);
    return keys;
}
async function callGemini(message, history, systemPrompt) {
    ...
    for (const apiKey of keys) {
        try {
            ...
            if (!res.ok) {
                const e = new Error(data.error?.message || `Gemini error: ${res.status}`);
            ...
            lastError = e;
            if (e.statusCode !== 429 && !String(e.message).includes('rate'))
                throw e;
        }
    }
    throw lastError || new Error('All Gemini keys failed');
}
async function callMimo(message, history, systemPrompt) {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.error?.message || `MIMO error: ${res.status}`);
...
async function handler(req, res) {
    ...
    try {
        ...
        if (url === '/api/chat' && method === 'POST') {
            ...
            if (!message?.trim())
                return err(res, 400, 'Message is required.');
            if (message.length > 10000)
                return err(res, 400, 'Message too long.');
            if (!['gemini', 'mimo'].includes(model))
                return err(res, 400, 'Invalid model.');
            const supabase = getSupabase();
            const { data: profile } = await supabase.from('profiles').select('plan').eq('id', user.id).single();
            const limits = getPlanLimits(profile?.plan || 'free');
            if (!limits.allowedModels.includes(model))
                return err(res, 403, `Plan does not support ${model}.`);
            ...
            if (rpcResult === -1)
                return err(res, 429, 'Daily limit reached.');
            ...
            try {
                ...
                try {
                    ...
                    return err(res, 502, e.message || 'All AI providers unavailable.');
                }
            }
            if (conversationId) {
        ...
        if (url === '/api/memories' && method === 'POST') {
            ...
            if (!memory_type || !content)
                return err(res, 400, 'memory_type and content required.');
        ...
        if (url === '/api/prompt-cache' && method === 'GET') {
            ...
            if (!cache_key)
                return err(res, 400, 'cache_key required.');
            const { data } = await getSupabase().from('prompt_cache').select('id, response, model').eq('user_id', user.id).eq('cache_key', cache_key).gt('expires_at', new Date().toISOString()).single();
            if (!data)
        ...
        if (url === '/api/prompt-cache' && method === 'POST') {
            ...
            if (!cache_key || !cachedResponse)
                return err(res, 400, 'cache_key and response required.');
            ...
            await getSupabase().from('prompt_cache').upsert({ user_id: user.id, cache_key, prompt_hash: hash, model: model || 'gemini', response: cachedResponse, expires_at: expiresAt }, { onConflict: 'user_id,cache_key,model' });
            return ok(res, {});
        }
        ...
        if (url === '/api/subscription/status' && method === 'GET') {
            ...
            if (!sub)
                return ok(res, { active: false, plan: 'free' });
        ...
        if (url === '/api/subscription/webhook' && method === 'POST') {
            ...
            if (existing)
                return ok(res, { message: 'Already processed.' });
            ...
            if (email) {
                ...
                if (profile) {
                    ...
                    const subData = { user_id: profile.id, paddle_customer_id: data?.customer?.id || '', paddle_subscription_id: data?.id || data?.subscription_id || '', status: isPro ? 'active' : 'canceled', plan: isPro ? 'pro' : 'free' };
                    if (data?.current_period_end)
                        subData.current_period_end = new Date(data.current_period_end * 1000).toISOString();
