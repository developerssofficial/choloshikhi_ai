// AI Provider logic for Vercel serverless functions
// Mirrors server/src/services/providers.ts

interface GeminiPart { text: string; }
interface GeminiContent { parts: GeminiPart[]; role: string; }
interface GeminiRequest { contents: GeminiContent[]; generationConfig?: { temperature?: number; maxOutputTokens?: number; topP?: number; topK?: number; }; }
interface GeminiResponse { candidates?: Array<{ content: { parts: GeminiPart[]; role: string; }; finishReason: string; }>; error?: { code: number; message: string; status: string; }; }
interface MimoRequest { model: string; messages: Array<{ role: string; content: string }>; temperature?: number; max_tokens?: number; }
interface MimoResponse { choices?: Array<{ message: { content: string; role: string }; finish_reason: string; }>; error?: { message: string; type: string; }; }
interface ChatHistoryItem { role: string; content: string; }
export interface ChatResult { message: string; model: string; cached: boolean; }
...
function isRateLimited(error: any): boolean {
  ...
  return ['rate limit', 'quota exceeded', 'too many requests', '429', 'resource_exhausted', 'daily limit'].some(i => errorStr.includes(i));
}
...
function formatGeminiMessages(history: ChatHistoryItem[], userMessage: string, systemPrompt: string): GeminiContent[] {
  ...
  for (const msg of history) {
    messages.push({ parts: [{ text: msg.content }], role: msg.role === 'assistant' ? 'model' : 'user' });
  }
  messages.push({ parts: [{ text: userMessage }], role: 'user' });
  return messages;
}

function formatMimoMessages(history: ChatHistoryItem[], userMessage: string, systemPrompt: string): MimoRequest['messages'] {
...
// Gemini key rotation (per-invocation, since serverless is stateless)
...
async function callGemini(userMessage: string, history: ChatHistoryItem[], systemPrompt: string): Promise<string> {
  const keys = getGeminiKeys();
  if (!keys.length) throw new Error('No Gemini API keys available');
  ...
  for (const apiKey of keys) {
    try {
      ...
      if (!response.ok) {
        const error: any = new Error(data.error?.message || `Gemini API error: ${response.status}`);
...
async function callMimo(userMessage: string, history: ChatHistoryItem[], systemPrompt: string): Promise<string> {
  ...
  if (!response.ok) throw new Error(data.error?.message || `MIMO API error: ${response.status}`);
...
export async function handleChat(userMessage: string, history: ChatHistoryItem[], modelPreference: string, mode?: string): Promise<ChatResult> {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      const response = targetModel === 'gemini'
        ? await callGemini(userMessage, history, systemPrompt)
        : await callMimo(userMessage, history, systemPrompt);
...
export function resetGeminiKeys() { /* no-op in serverless */ }
export function getProviderStatus() {
  return {
    gemini: getGeminiKeys().length,
    mimo: process.env.MIMO_API_KEY ? 1 : 0,
  };
}
